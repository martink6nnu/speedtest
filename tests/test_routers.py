...
class TestSpeedRouter:
    ...
    def test_speed_endpoint_success(self, mock_wait_for, mock_create_subprocess, test_client):
        ...
        assert response_data["download_speed"] == round(99478925.14 / 1_000_000, 2)  # 99.48
        assert response_data["upload_speed"] == round(78648744.10 / 1_000_000, 2)     # 78.65
    ...
    @patch("src.repositories.requester.asyncio.create_subprocess_exec")
    @patch("src.repositories.requester.asyncio.wait_for")
    def test_speed_endpoint_error_scenarios(self, mock_wait_for, mock_create_subprocess, test_client):
    ...
    @patch("src.repositories.requester.asyncio.create_subprocess_exec")
    @patch("src.repositories.requester.asyncio.wait_for")
    def test_speed_endpoint_different_data(self, mock_wait_for, mock_create_subprocess, test_client):
        ...
        response = test_client.get("/speed")
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["download_speed"] == round(50000000.0 / 1_000_000, 2)  # 50.0
